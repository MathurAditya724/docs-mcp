...
const fastapi: SentrySkill = {
  ...
  features: [
    ...
    },
    {
      ...
      name: "User Feedback",
      setup:
        "In your FastAPI exception handler, retrieve the event ID with sentry_sdk.last_event_id() and render it into an HTML response that loads the Sentry browser SDK and calls Sentry.showReportDialog({ eventId }) to display a feedback modal prompting for name, email, and description.",
      slug: "user-feedback",
    },
  ],
  gettingStarted:
